...
class Instagram implements ExperimentsInterface {
  ...
  $debug = false, $truncatedDebug = false, array $storageConfig = []) {
    ...
    foreach ($extensions as $ext) {
      if (!@extension_loaded($ext)) {
        throw new \InstagramAPI\Exception\InternalException(sprintf(
                'You must have the "%s" PHP extension to use the Instagram API library.', $ext
        ));
      }
    }
  ...
  $username, $password, $force_login = FALSE, $appRefreshInterval = 1800) {
    if (empty($username) || empty($password)) {
      throw new \InvalidArgumentException('You must provide a username and password to login().');
    }

    return $this->_login($username, $password, $force_login, $appRefreshInterval);
  }
  ...
  $username, $password, $forceLogin = false, $appRefreshInterval = 1800) {
    if (empty($username) || empty($password)) {
  ...
  $username, $password, $twoFactorIdentifier, $verificationCode, $appRefreshInterval = 1800) {
    if (empty($username) || empty($password)) {
  ...
  $username, $password) {
    ...
    $this->device = new Devices\Device(
            Constants::IG_VERSION, Constants::VERSION_CODE, Constants::USER_AGENT_LOCALE, $savedDeviceString
    );
  ...
  $justLoggedIn, $appRefreshInterval = 1800) {
  ...
  $experiment, $param) {
    return isset($this->experiments[$experiment][$param]) && in_array($this->experiments[$experiment][$param], ['enabled', 'true', '1']);
  }
  ...
  $experiment, $param, $default = null) {
    return isset($this->experiments[$experiment][$param]) ? $this->experiments[$experiment][$param] : $default;
  }
