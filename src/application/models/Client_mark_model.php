...
class Client_mark_model extends CI_Model {
    ...
    function load_doorig_follows($client_id) {
        ...
        $this->db->select('sum(`follows`) as Count')->from('reference_profile');
    ...
    function get_all_by_status($status_id) {
        ...
        $this->db->where('client_status_list.client_status_id', $status_id);
    ...
    function update_last_acctess($client_id, $time) {
        $data = array(
            'last_access' => "'$time'"
        );
        $this->db->where('client_id', $client_id);
