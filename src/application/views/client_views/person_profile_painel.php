<!-- PERSON PROFILE DATAS -->
<div class="card">
    <div class="card-body">
        <div class="container sensitive_painel">
            <div class="row">
                <div class="col-md-12">
                    <div class="sec-title text-center">
                        <h2>Sua Marca</h2>
                        <span class="border"></span>
                    </div>
                </div>                                        
            </div>
            <div class="row">
                <div class="col-md-1 col-sm-6 col-xs-12"></div>
                <div class="col-md-3 col-sm-6 col-xs-12">
                    <div class="text-center">
                        <a id="ig-profile-url" href="" target="_blank">
                            <img id="ig-profile-picture-url" class="img-profile-client" src="<?php echo base_url()?>assets/images/users/default-user.png" alt="photo">
                            <p id="ig-profile-name" style="color:black;font-size:1.5em">@profile</p>
                        </a>
                    </div>
                </div>
                <div class="col-md-7 col-sm-6 col-xs-12">                                            
                    <div class="row">                                                
                        <div class="col-md-4 col-sm-12 col-xs-12 text-center p-0">
                            <p class="m-b-0">Posts</p>
                            <h6 id="ig-profile-amount-post" class="text-muted"></h6>
                        </div>
                        <div class="col-md-4 col-sm-12 col-xs-12 text-center p-0">
                            <p class="m-b-0 ">Seguidores</p>
                            <h6 id="ig-profile-amount-followers" class="text-muted"></h6>
                        </div>
                        <div class="col-md-4 col-sm-12 col-xs-12 text-center p-0" >
                            <p class="m-b-0 " >Seguindo</p>
                            <h6 id="ig-profile-amount-following" class="text-muted"></h6>
                        </div>
                    </div>
                    <div class="row m-top-20">                                                
                        <div class="col-md-12 col-sm-12 col-xs-12 text-justify p-0">
                            <p id="ig-profile-description" class="m-b-0"></p>
                        </div>                                                
                    </div>

                    <div class=" m-top-20">                                                
                        <input id="activate-account" type="checkbox" checked data-toggle="toggle" data-on="<i class='fa fa-play'></i> Play" data-off="<i class='fa fa-pause'></i> Pause">                                              
                    </div>

                </div>
                <div class="col-md-1 col-sm-6 col-xs-12"></div>                                    
            </div>
        </div>
    </div>
</div>
