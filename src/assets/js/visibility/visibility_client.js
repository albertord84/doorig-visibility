...
function select_profile_from_search(input_profile_id,table_match_id,prof_name) {
    selected_profile = find_match_in_list(match_profiles,prof_name);    
    $(input_profile_id).val(prof_name);
    $(table_match_id).empty();
}

function find_match_in_list(list_profiles,prof){
    i=0;
...
function show_profile_in_view(container,profile){
    $.ajax({
        ...
        dataType: 'json',        
        success: function (response) {
            datas = find_match_in_list(response['users'],profile); 
            datas = datas.user;
